...
def detect_main_ice_break(video_path, output_dir, output_formats=None):
    ...
    colors = {
        ...
        'accent': (16, 137, 62),      # Green
        'highlight': (230, 230, 0),   # Yellow
        'background': (45, 45, 45),   # Dark gray
        'text': (240, 240, 240),      # Off-white
        'grid': (100, 100, 100),      # Medium gray
        'alert': (220, 53, 69),       # Red
        'water': (32, 99, 155)        # Deep blue
    }
    ...
    for fmt in output_formats:
        if fmt in codec_map:
            ...
            print(f"Initialized {fmt.upper()} writer: {output_path}")
    ...
    for _ in tqdm(range(total_frames), desc="Processing video"):
        ...
        # Add a dilated version of the mask for better visualization
        ...
        if contours:
            ...
            # Add heading
            cv2.putText(
                result_frame, "ICE BREAK MEASUREMENTS", 
            ...
            cv2.putText(
                result_frame, f"Width: {actual_width_pixels} px ({estimated_width_km:.2f} km)", 
                (20, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.7, colors['text'], 1, cv2.LINE_AA
            )
            cv2.putText(
                result_frame, f"Distance from Shore: {shore_distance_km:.2f} km", 
            ...
            cv2.putText(
                result_frame, f"Area: {area_sq_km:.2f} sq km", 
                (20, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.7, colors['text'], 1, cv2.LINE_AA
            )
            
            # Add "ANALYZING" text with animated dots
            dots = "." * (frame_count % 4)
            cv2.putText(
                result_frame, f"ANALYZING{dots}", 
            ...
            if len(width_history) > 1:
                ...
                # Display title
                cv2.putText(result_frame, "TREND ANALYSIS", 
    ...
    if len(width_history) > 0:
        try:
            ...
            print(f"Trend analysis saved to {trend_image_path}")
        except Exception as e:
            print(f"Warning: Failed to save trend analysis image: {e}")
    ...
    for fmt in video_writers.keys():
        ...
        print(f"- {fmt.upper()} Video: {output_path} ({file_size_mb:.2f} MB)")
    
    if len(width_history) > 0 and os.path.exists(f"{output_base}_trend.png"):
        trend_size_kb = os.path.getsize(f"{output_base}_trend.png") / 1024
        print(f"- Trend Image: {output_base}_trend.png ({trend_size_kb:.2f} KB)")
...
def main():
    # Define paths
    video_path = r"C:\Users\x\Music\ice break\ic.mp4"  # Change to your video path
    output_dir = r"C:\Users\x\Music\ice break\output"  # Your specified output directory
    
    # List of output formats - you can modify this to include your preferred formats
    output_formats = ['mp4', 'avi', 'mov', 'wmv']
